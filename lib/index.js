...
srcPath = (srcPath.lastIndexOf('/') != (srcPath.length-1)) ? srcPath + '/' :srcPath
destPath = (destPath.lastIndexOf('/') != (destPath.length-1)) ? destPath + '/' :destPath
...
async function start() {
  try {
    ...
    srcList.forEach(async function(file, index) {
      bar[file.name] = new _cliProgress.Bar(
        { format: file.name.green + "[{bar}] {percentage}% | ETA: {eta}s" },
        _cliProgress.Presets.shades_classic
      );
      bar[file.name].start(100, 0);
      ...
      await myFTP.put(
        ...
        destFTP,
        destProtocol == "ftp" ||
          (destProtocol == "sftp" && srcProtocol == "ftp") || (destProtocol == "sftp" && srcProtocol == "sftp")
